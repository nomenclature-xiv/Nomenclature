...
public class IdentityService : IHostedService
{
    ...
    private void ChatGuiOnChatMessage(XivChatType type, int timestamp, ref SeString sender, ref SeString message, ref bool isHandled)
    {
        ...
        {
            //it's you! probably.
            string sendername = ((TextPayload)payloads[0]).Text;
        ...
        else if(payloads.Count is 3)
        {
            ...
            if (ChangeName(ref payloads, senderchar, self, icon))
            {
    ...
    private bool ChangeName(ref List<Payload> payloads, Character character, Character self, string icon = "")
    {
